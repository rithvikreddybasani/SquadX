...
const BACKEND_URL = "https://squad-x-33wj.vercel.app/";
...
const SocketProvider = ({ children }) => {
    ...
    const handleTypingStart = useCallback(({ username }) => {
        setUsers((prev) =>
            prev.map((user) =>
                user.username === username ? { ...user, typing: true } : user
            )
        );
    }, [setUsers]);
    ...
    const handleTypingStop = useCallback(({ username }) => {
        setUsers((prev) =>
            prev.map((user) =>
                user.username === username ? { ...user, typing: false } : user
            )
        );
    }, [setUsers]);
    ...
    useEffect(() => {
        ...
        socket.on(ACTIONS.TYPING_START, handleTypingStart),
        socket.on(ACTIONS.TYPING_PAUSE, handleTypingStop);
